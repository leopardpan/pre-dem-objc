...
+ (void)setNetworkClient:(PREDNetworkClient *)networkClient {
    [self sharedSender].networkClient = networkClient;
}

+ (PREDNetworkClient *)networkClient {
    return [self sharedSender].networkClient;
}
...
- (void)sendLog {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        [_networkClient postPath:@"http-stats/i" data:dataToSend headers:headers completion:^(PREDHTTPOperation *operation, NSData *data, NSError *error) {
